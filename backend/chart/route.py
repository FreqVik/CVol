...
@router.get("/returns")
async def get_chart_returns(symbol: str = "BTC/USDT", timeframe: str = "4h", limit: int = 100):
...
async def get_realized_vol(symbol: str = "BTC/USDT", timeframe: str = "4h", limit: int = 100, window: int = 20):
    df = chart_service.get_or_create_dataframe(symbol=symbol, timeframe=timeframe, limit=max(limit, 100))
    df, _ = chart_service.append_new_data(symbol=symbol, timeframe=timeframe)
...
@router.post("/append")
async def append_chart_data(symbol: str = "BTC/USDT", timeframe: str = "4h", fetch_limit: int = 10):
